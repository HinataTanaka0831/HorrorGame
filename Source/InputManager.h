...
public:
	...
	// マウスボタンが押下され続けている状態の検知
	// 入力: mouseCode(MOUSE_INPUT_LEFT等のビットフラグ) / 出力: 押下中ならtrue、それ以外はfalse / 副作用: なし
	bool CheckPressMouseClick(int mouseCode);

	// マウスボタンが押された瞬間の検知
	// 入力: mouseCode(MOUSE_INPUT_LEFT等のビットフラグ) / 出力: 押下瞬間ならtrue、それ以外はfalse / 副作用: なし
	bool CheckTriggerMouseClick(int mouseCode);

	// マウスボタンが離された瞬間の検知
	// 入力: mouseCode(MOUSE_INPUT_LEFT等のビットフラグ) / 出力: 離された瞬間ならtrue、それ以外はfalse / 副作用: なし
